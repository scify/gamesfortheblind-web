...
$item = $this->items;
$type = JRequest::getInt('virtuemart', '1');

if(count($item)){
	if($type == 1){
		$part_link = JURI::root()."index.php?option=com_virtuemart&view=category&virtuemart_category_id=";
	}
	elseif($type == 2){
		$part_link = JURI::root()."index.php?option=com_virtuemart&view=productdetails&virtuemart_product_id=";
	}
}
...
window.addEvent('domready', function () {
	if (document.getElementById('virtuemart').value == 0) {
	  document.getElementById('virtuemart').value = 1;
...
<div class="metatagsvirtuemart-content">			
  ...
  <div class="row-fluid">
      <div class="span12">
       <table class="table table-striped table-bordered" id="metatags-virtuemart">  
		<thead>
		...
		$limistart	= $app->getUserStateFromRequest('com_ijoomla_seo.virtuemart'.'.list.start', 'limitstart');
		$limit		= $app->getUserStateFromRequest('com_ijoomla_seo.virtuemart'.'.list.limit', 'limit');
		$k = $limistart+1;
		...
		for($i=0; $i<count($this->items); $i++){
			...
			$page_title["page_title"] = $item->customtitle;
